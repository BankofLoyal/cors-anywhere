...
function isValidOrigin(origin) {
  for (var i=0; i<corsAnywhere.originWhitelist.length; i++) {
    if (origin.match(corsAnywhere.originWhitelist[i]) != null)
      return true;
  }
  return false;
}
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originWhitelist: ['^https://[a-z0-9.]*.gravtee.com$','https://api.sendgrid.com/v3/mail/send', '^https://[a-z]*.+[a-z]*.gravty.io$', '^https://[a-z]*.+[a-z]*.gravty.info$', 'https://gravty.shoppersstop.net'],            // If non-empty, requests not from an origin in this list will be blocked.
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.originWhitelist.length && isValidOrigin(origin)) {
      res.writeHead(403, 'Forbidden', cors_headers);
